...
def render_run_domain_surfdata_sh(cfg: dict, scripts_dir: Path, exp_root: Path) -> str:
    ...
    lines.append("export TES_DATA_GROUP_ID BASE_DOMAIN_FILE SURFDATA_FILE")
...
def render_run_forcing_sbatch(cfg: dict, scripts_dir: Path, exp_root: Path) -> str:
    ...
    qos = scheduler.get("qos", partition)
    nodes = scheduler.get("nodes", 1)
    time_limit = scheduler.get("time", "2:00:00")
    mem = scheduler.get("mem", "128GB")
    tasks = scheduler.get("tasks", 2)
    cpus_per_task = scheduler.get("cpus_per_task", 1)
    bootstrap_lines = scheduler.get("bootstrap", [])
    _, _, tes_data_group_id = _derive_template_vars_from_cfg(cfg)
    ...
    lines.append(f"#SBATCH -n {tasks}")
    lines.append(f"#SBATCH -c {cpus_per_task}")
    ...
    lines.append("")
    lines.append('python3 -c "from mpi4py import MPI" 2>/dev/null || {')
    lines.append('  echo "ERROR: mpi4py not found in ${PYTHON_ENV:-tes_aoi}; install with: conda install -c conda-forge mpi4py"')
    lines.append("  exit 1")
    lines.append("}")
    ...
    lines.append("")
    lines.append(f'SRUN_QOS="${{SCHED_QOS:-{qos}}}"')
    lines.append(f'SRUN_PARTITION="${{SCHED_PARTITION:-{partition}}}"')
    lines.append(f'SRUN_NODES="${{SCHED_NODES:-{nodes}}}"')
    lines.append(f'SRUN_TIME="${{SCHED_TIME:-{time_limit}}}"')
...
def render_create_uelm_adspin_sh(cfg: dict, exp_root: Path) -> str:
    ...
    case_size_type = str(cfg.get("case_size_type", "small")).lower()

    # Derive template-like variables to mirror the template/finalspin style
    ...
    lines.append("")
    lines.append('CASEDIR="$KMELM_CASE_ROOT/${TES_DOMAIN_FORCING_GROUP_ID}/uELM_${EXPID}_${CASE_COMPSET}"')
    ...
    lines.append("")
    # Scientific configuration for adspin
    ...
    lines.append("EOF")
    lines.append("")
    # Computational resources (conditional by case size)
    lines.append("# Computational resources")
    if case_size_type == "large":
        lines.append('./xmlchange NTASKS_LND="1280"')
        lines.append('./xmlchange NTASKS_ATM="100"')
        lines.append('./xmlchange NTASKS_CPL="100"')
        lines.append('./xmlchange MAX_MPITASKS_PER_NODE="128"')
        lines.append('./xmlchange JOB_WALLCLOCK_TIME="24:00:00"')
    else:
