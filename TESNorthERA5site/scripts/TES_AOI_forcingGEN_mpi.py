...
def AOI_forcing_save_1d(input_path, file, AOI, AOI_points, output_path, chunk_size=DEFAULT_CHUNK_SIZE):
    ...
    aoi_pts = np.unique(np.asarray(AOI_points).reshape(-1))
    flat_ids = np.asarray(grid_ids).reshape(-1)
    AOI_idx = np.where(np.isin(flat_ids, aoi_pts))[0]
    if AOI_idx.size == 0:
        raise RuntimeError(f"No matching AOI gridIDs found for file: {source_file}")
    ...
    for name, dimension in src.dimensions.items():
        if name != 'ni' and name != 'gridcell':
            ...
            dst.createDimension(name, aoi_pts.size)
